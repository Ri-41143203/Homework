...
透過friend管理多項式的輸入輸出，避免外部程式直接存取私有成員；同時將Add、Mult、Eval封裝於Polynomial類別內，使程式結構清楚、方便使用。實作過程中發現，如果輸入多項式項數較多(>5)，程式會出錯並算出錯誤結果，感覺可能是我newTerm沒寫好。透過這次作業，更熟悉了C++類別封裝、friend的使用方式，以及動態記憶體管理與陣列操作。
